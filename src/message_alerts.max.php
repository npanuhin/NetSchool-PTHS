...
try {
	...
	$db->query('INSERT IGNORE INTO `messages` (`user_id`, `msg_data`) VALUES (?i, ?s)',
		$person['id'],
		'[
			{
				"id": 1,
	...
	$messages = $db->getOne('SELECT `msg_data` FROM `messages` WHERE `user_id` = ?i', $person['id']);
	...
	foreach ($messages as $index => $message) {
		...
		<div class="message_alert" id="message_alert_<?php echo $message['id']?>">
			...
			if ($message['expires'] == 'once-seen') {
				...
				if ($message['expires_at'] <= 0) unset($messages[$index]);
			}
	...
	// print_r($e);
	telegram_log("Error in message_alerts\n\n" . $e->getMessage());
}
?>
