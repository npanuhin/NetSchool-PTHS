			...
			<ul>
				...
				foreach ($announcements as $announcement) {
					...
					$article = preg_replace(
						'/(' . get_school_class_regex(trim($person['class'])) . ')(?![^<]*>|[^<>]*<\/)/imu',
						'<span class="school_class" title="Это ваш класс (тут нет никакой ссылки)">\1</span>',
						nl2br(trim($announcement['text']))
					);
					?>

					<li class="announcement" announcement_id="<?php echo $announcement_id ?>" title="<?php echo $title . ' (' . $author . ', ' . $date->format('d') . ' ' . $months_genetive[$date->format('m') - 1] . ')' ?>">
